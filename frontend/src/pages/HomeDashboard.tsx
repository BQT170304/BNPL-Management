...
const fmtFull = (n: number) => (n >= 1_000_000 ? `${(n / 1_000_000).toFixed(1)}M ₫` : `${n.toLocaleString("vi-VN")} ₫`);
...
export function HomeDashboard({ profileId, cif }: Props) {
  ...
  useEffect(() => {
    ...
    Promise.all([getAnalysis(profileId), getForecast(cif).catch(() => null)])
      .then(([m, f]) => {
        setMetrics(m);
        setForecast(f);
      })
  ...
  const chartData = (() => {
    ...
    return [...histMonths.map((h) => ({ ...h, forecast: false as const })), ...fcastMonths];
  })();
  ...
  const healthLabel =
    metrics.overall_health_score >= 70 ? "Tốt" :
    metrics.overall_health_score >= 50 ? "Cần chú ý" : "Cần cải thiện";
  const healthColor =
    metrics.overall_health_score >= 70 ? "rgba(52,211,153,0.25)" :
    metrics.overall_health_score >= 50 ? "rgba(251,191,36,0.25)" : "rgba(248,113,113,0.25)";
  const healthTextColor =
    metrics.overall_health_score >= 70 ? "#6EE7B7" :
    metrics.overall_health_score >= 50 ? "#FDE68A" : "#FCA5A5";

  return (
    <div className="home-page">
      <div className="hero-header">
        <div className="hero-text">
          <span className="hero-greeting">Xin chào, Huy</span>
          <span className="hero-subtitle">Tổng quan tài chính của bạn</span>
        </div>
        <span className="hero-health-badge" style={{ background: healthColor, color: healthTextColor }}>
          {healthLabel}
        </span>
      </div>
            ...
            {metrics.goals.length > 0 && (
              <div className="home-section-card">
                ...
                {metrics.goals.map((g: GoalMetricOut) => {
                  const statusColor =
                    g.grs > 50 ? "var(--color-danger)"
                    : g.grs > 20 ? "var(--color-warning)"
                    : "var(--color-success)";
                  const statusLabel =
                    g.grs > 50 ? "Trễ tiến độ"
                    : g.grs > 20 ? "Cần tăng tốc"
                    : "Đúng lộ trình";
                  return (
            ...
            <div className="home-section-card">
              <div className="forecast-header">
                ...
                <div className="forecast-pills">
                  <span
                    className="pill"
                    style={{ color: forecast.next_30_net >= 0 ? "var(--color-success)" : "var(--color-danger)" }}>
                    30 ngày: {forecast.next_30_net >= 0 ? "+" : ""}
                    {fmtM(forecast.next_30_net)}
                  </span>
                  <span
                    className="pill"
                    style={{ color: forecast.next_90_net >= 0 ? "var(--color-success)" : "var(--color-danger)" }}>
                    90 ngày: {forecast.next_90_net >= 0 ? "+" : ""}
                    {fmtM(forecast.next_90_net)}
                  </span>
                </div>
              </div>
